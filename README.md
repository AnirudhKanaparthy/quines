...
For interpreter language, just run it with an interpreter. For a compiled one, just compile and run it as an executable. If you are in Linux you can use `diff` command to get the difference between the original file and the output of the program.
Example:
```shell
cc -o quine 1.c && ./quine > 1_clone.c && diff -s 1.c 1_clone.c
```
